...
class Zomato:
    ...
    def restaurant_search_by_city(self, city_id=1, cuisines="", offset=0):
        """
        Takes city_id, cuisine and offset as input.
        Returns a list of Restaurant IDs.
        """
        cuisines = "%2C".join(cuisines.split(","))
        
        headers = {'Accept': 'application/json', 'user-key': self.user_key}
        r = (requests.get(base_url + "search?entity_id=" + str(city_id) + "&entity_type=city" + "&start=" + str(offset) + "&cuisines=" + str(cuisines) + "&sort=rating&order=desc", headers=headers).content).decode("utf-8")
        return r#a = ast.literal_eval(r)
        
    async def restaurant_search_by_city_async(self, city_id=1, cuisines="", offset=0):
        """
        Takes city_id, cuisine and offset as input.
        Returns a list of Restaurant IDs.
        """
        ...
        async with ClientSession() as session:
            async with session.get(base_url + "search?entity_id=" + str(city_id) + "&entity_type=city" + "&start=" + str(offset) + "&cuisines=" + str(cuisines) + "&sort=rating&order=desc", headers=headers) as response:
